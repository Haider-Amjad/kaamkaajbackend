...
router.post('/add_serviceCategory', function (req, res) {
    ...
    serviceCategory.getServiceCategoryByName(serviceCategoryform.name, function (err, result) {
        if (err) {
            console.log(err);
            return res.status(500).json({
                Message: "Error in Connecting to DB 00",
                status: false
            });
        } 
        else if (result!="")
        {
            ...
            serviceCategory.addServiceCategory(serviceCategoryform, function (err, serviceCategorys) {
                if (err)
                {
                    console.log("error"+err)
                    return res.status(500).json({
                        Message: "Error in Connecting to DB11",
                        status: false
                    });
                    ...
                    var result = serviceCategorys.toObject();
                    result.status = true;
                    return res.json(result);
                }
