...
router.post('/add_bookingSetting', function (req, res) {
    ...
    bookingSetting.addbookingSetting(bookingSettingform, function (err, bookingSetting) {
        if (err) 
        {
            ...
            console.log(bookingSetting.serviceProviderEmail)
        // {
        //     serviceP rovier.updateServiceProvider(bookingSettingform.serviceProviderEmail,{profile_completed:true},{new: true},function (err,serviceProvider)
        //     {
        //         if(err)
        //         {
        //             console.log(err);
...
router.get('/get_settings', function (req, res) {
    bookingSetting.getbookingSetting(function (err, result) {
        if (err)
            return res.status(500).json({
                Message: "Error in Connecting to DB",
                status: false
